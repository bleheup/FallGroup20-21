...
class ActorNetwork(keras.Model):
    ...
    def sample_action_dist(self, state):
        ...
        # This agent does not use the returned actions for loss/cost calculation, so we don't have to worry about numpy functions not being in scope of the tf computation graph
        # (Not even sure if it would matter anyways since it's just a probability sample, but good to know anyways)

        ## FIGURE OUT HOW TO CONVERT PROBS TO NP ARRAY
        ...
        #action = prob_dist.sample(NUM_ACTIONS)  # TODO: Implement sampling without replacement!!!
        #log_prob = prob_dist.log_prob(action)


        # Returns sampled action, log prob of that sampled action, probabilities of all actions, and log probs of all actions
        log_prob = []  # unused
        return action_tensor, log_prob, probs, log_probs
...
class AgentDiscreteSAC(Player):
    ...
    def choose_action(self, observation):
        ...
        if self.is_learning:
            ...
            #print("sampled action =", action_combos)
            #print("sampled action.numpy() =", action_combos.numpy())
    ...
    def learn(self, n_steps=1):
        ...
        for _ in range(n_steps):
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                #print("next_probs =", next_probs) if DEBUG else None
                #print("min/mean/max =\n{}\n{}\n{}".format( tf.reduce_min(next_probs, axis=1), tf.reduce_mean(next_probs, axis=1), tf.reduce_max(next_probs, axis=1) )) if DEBUG else None
                ...
                for action_num in range(NUM_ACTIONS):
                    ...
                    for g, n in zip(group, node):
                        q1 += critic1_reshaped[:, g, n-1]  # Everglades nodes use 1-based indexing (I think??), so convert to 0-based to access correct index
...
def action_combo_to_arr(action_combo):
    ...
    node = 1+(action_combo % NUM_NODES)  # Everglades nodes use 1-based indexing (I think??), so convert node from 0-based to 1-based for action
...
#LEARN_DELAY = -1  # Number of episodes before model starts sampling memory and learning
